...
public class SerializableGuidDrawer : PropertyDrawer
{
    ...
    void RegenerateGuid(SerializedProperty property)
    {
        ...
        var path = property.propertyPath; // e.g. "myGuidField"
        ...
        foreach (var target in targets)
        {
            ...
            // find parts for this particular target
            var parts = new SerializedProperty[GuidParts.Length];
            bool missing = false;
            for (int i = 0; i < GuidParts.Length; i++)
            {
                parts[i] = so.FindProperty(path + "." + GuidParts[i]);
                if (parts[i] == null) { missing = true; break; }
            }
            if (missing) continue;
